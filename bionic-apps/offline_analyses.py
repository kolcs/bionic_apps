...
from .handlers.hdf5 import init_hdf5_db
...
def test_eegdb_within_subject(base_dir=DATA_PATH, f_type=FeatureType.HUGINES,
                              window_len=.05, window_step=.01, *,
                              n_splits=5,
                              classifier='ensemble',
                              classifier_kwargs=None,
                              ch_mode='all', ep_mode='distinct',
                              db_file='database.hdf5', log_file='out.csv'):
    if classifier_kwargs is None:
        classifier_kwargs = {}
    ...
    generate_db(db_file, f_type, base_dir, window_len, window_step,
                ch_mode=ch_mode, ep_mode=ep_mode)
